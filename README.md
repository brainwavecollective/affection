...
What is not felt cannot be communicated. If there is nothing experienced there is nothing to express. **Affection understands speech as it is spoken, amplifies and emotional content and translates into mood, and defines a rich dynamic expereience that can be expressed through any outlet.** Although output can be consumed for any purpose, there are examples here that cover two expressive outlets:  
 1) Using cinematic color to communicate feelings, through an external [physical device](https://github.com/brainwavecollective/reachy-eyes/) with engineered electronics, compiled firmware, and an embedded ML model
 2) Various aspects of body language, through posture and antenna movement demonstrated in a dedicated version of the [Reachy Mini Conversation App](https://github.com/brainwavecollective/reachy_mini_conversation_app/tree/body-language)
...
This project extends the **Reachy Mini robot by Pollen Robotics** (or any other robot) with embedded ML hardware, multi-layer systems design, and artistic insight to express realtime edge emotion: 
 ...
 - Interactive dialogue for the eyes example is generated by fork of [Reachy Mini Conversation App](https://github.com/brainwavecollective/reachy_mini_conversation_app/tree/Affection) by **Pollen Robotics**
 - Mood derived from **realtime emotional affect extraction engine** with **cinematic amplification** *(THIS REPO)*
 - UPDATE! A full [standalone version of the affect-engine](https://github.com/brainwavecollective/affect-engine/tree/main/affect_engine) has also been published. 
...
Affection backs claims with a working, tuned, end-to-end system, encompassing affective computing, real-time inference, HRI (human–robot interaction), and edge rendering. Demos cover expression through both eye color as well as body language, two separate manifolds that can be combined into a single, integrated experience.
...
Although the example in this project uses an external embedded device, the 8GB NVIDIA Jetson Orin Nano has more than enough capacity to directly take on the workload of multiple outputs. The embedded device proves that a small, inexpensive add-on can fundamentally change the experience of interacting with a robot. Plug in the expressive module, allow the engine to identify and express feelings, and add life to an existing robot through multiple outlets. 

**The affect-engine is fully decoupled. Eye color and body languages are limited example applications.**
...
| Fast Loop Full Render    | typicall < 5ms                       |
| Slow Loop Full Render    | 1.6–1.8s                             |


## How it works

Affection bridges three interconnected layers: language understanding at the edge, interpretive display through embedded hardware, and embodied presence in physical robotics.
...
Understanding is built in two loops. A limbic **fast loop** scores each sentence as it is spoken, capturing momentary emotional bursts. A reflective **slow loop** runs a local LLM (4B Nemotron Mini from Ollama) as background recalculation of the affective baseline. This allows for both precise understanding as well as subjective interpretaion. The two are blended in real time so the eyes respond to emotional spikes while maintaining the general feel of the conversation. The immediate reactive emotion is expressed, and then we reflect and refine.

Weak emotional signals are amplified and optionally aligned with established cinematic mood. A tunable "drama knob" let's a user choose how much to rely on predefined emotionally weighted visual anchors (Range: raw truth, to total cinematic license, and beyond extreme exaggeration). 
...
For a detailed breakdown of this pipeline, see the [Affect Engine overview](EMOTION_ENGINE.md). 

### EXPERIENCED: Embedded Interpretive Display
...
Expression becomes more meaningful when it is integrated into a single experience. Emotional vectors computed at the edge and interpreted in embedded firmware are ultimately rendered through multiple channels:

1) Dynamically expressive eyes  
2) Body Language   
...
There is another example showing integration with the existing [Reachy Mini Conversation App](https://github.com/brainwavecollective/reachy_mini_conversation_app/tree/body-language) and that can be run as it's own client. Note the NCR VAD model is non-distributable so you will need to download the data separately before running the app (It can be modified to remve the prereq to auto-download, which would allow it to be run as a community app).

For running the engine as a standalone server, run these processes in **3 separate terminals**:
```
...
As you talk to your robot it will demonstrate behavior change based on the nature of the conversation.
